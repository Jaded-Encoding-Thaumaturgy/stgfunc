...
class Core(vs.Core):
  ...
  def Pad(self, src: vs.VideoNode, left: int, right: int, top: int, bottom: int) -> vs.VideoNode:
    return self.Resample(src, src.width + left + right, src.height + top + bottom, -left, -top, src.width + left + right, src.height + top + bottom, kernel="point", **fmtc_args)
  ...
  def ThrMerge(self, src: vs.VideoNode, ref1: BasicClip, ref2: BasicClip = None, thr: float = 0.0009765625, elast: float = None) -> vs.VideoNode:
...
class internal:
  ...
  def basic(
    ...
    if short_time:
      ...
      mvbw_vmulti = core.MRecalculate(supersoft, mvbw_vmulti, chroma=color, overlap=2, blksize=4, thsad=me_sad[0], **mrecalculate_args)
      mvbw_vmulti = core.MRecalculate(supersoft, mvbw_vmulti, chroma=color, overlap=1, blksize=2, thsad=me_sad[1], **mrecalculate_args)

      mvfw_vmulti = core.MAnalyze(supersoft, chroma=color, overlap=4, blksize=8, **manalyze_args)
      mvfw_vmulti = core.MRecalculate(supersoft, mvfw_vmulti, chroma=color, overlap=2, blksize=4, thsad=me_sad[0], **mrecalculate_args)
      mvfw_vmulti = core.MRecalculate(supersoft, mvfw_vmulti, chroma=color, overlap=1, blksize=2, thsad=me_sad[1], **mrecalculate_args)
    else:
      ...
      mvbw_vmulti = core.MRecalculate(supersoft, mvbw_vmulti, chroma=color, overlap=32, blksize=64, thsad=me_sad, **mrecalculate_args)
      mvbw_vmulti = core.MRecalculate(supersoft, mvbw_vmulti, chroma=color, overlap=16, blksize=32, thsad=me_sad, **mrecalculate_args)
      mvbw_vmulti = core.MRecalculate(supersoft, mvbw_vmulti, chroma=color, overlap=8, blksize=16, thsad=me_sad, **mrecalculate_args)
      mvbw_vmulti = core.MRecalculate(supersoft, mvbw_vmulti, chroma=color, overlap=4, blksize=8, thsad=me_sad, **mrecalculate_args)
      mvbw_vmulti = core.MRecalculate(supersoft, mvbw_vmulti, chroma=color, overlap=2, blksize=4, thsad=me_sad, **mrecalculate_args)

      mvfw_vmulti = core.MAnalyze(supersoft, chroma=color, overlap=64, blksize=128, **manalyze_args)
      mvfw_vmulti = core.MRecalculate(supersoft, mvfw_vmulti, chroma=color, overlap=32, blksize=64, thsad=me_sad, **mrecalculate_args)
      mvfw_vmulti = core.MRecalculate(supersoft, mvfw_vmulti, chroma=color, overlap=16, blksize=32, thsad=me_sad, **mrecalculate_args)
      mvfw_vmulti = core.MRecalculate(supersoft, mvfw_vmulti, chroma=color, overlap=8, blksize=16, thsad=me_sad, **mrecalculate_args)
      mvfw_vmulti = core.MRecalculate(supersoft, mvfw_vmulti, chroma=color, overlap=4, blksize=8, thsad=me_sad, **mrecalculate_args)
      mvfw_vmulti = core.MRecalculate(supersoft, mvfw_vmulti, chroma=color, overlap=2, blksize=4, thsad=me_sad, **mrecalculate_args)
  ...
  @staticmethod
  def deringing(
      core: Core, src: vs.VideoNode, ref: BasicClip, radius: int, h: float,
      sigma: float, mse: tuple[float, float], hard_thr: float, block_size: int, block_step: int, group_size: int, bm_range: int,
      bm_step: int, ps_num: int, ps_range: int, ps_step: int, lowpass: list[float], color: bool, matrix: int
  ):
  ...
  @staticmethod
  def destaircase(
      core: Core, src: vs.VideoNode, ref: BasicClip, radius: int, sigma: float,
      mse: tuple[float, float], hard_thr: float, block_size: int, block_step: int, group_size: int, bm_range: int, bm_step: int,
      ps_num: int, ps_range: int, ps_step: int, thr: float, elast: float, lowpass: list[float], matrix: int
  ):
  ...
  @staticmethod
  def deblocking(
      core: Core, src: vs.VideoNode, ref: BasicClip, radius: int, h: float, sigma: float,
      mse: tuple[float, float], hard_thr: float, block_size: int, block_step: int, group_size: int, bm_range: int, bm_step: int,
